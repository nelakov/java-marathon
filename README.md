...
> A 19-day, hands-on Java learning marathon — **one package per day** — progressing from
> language basics to object-oriented design, collections, file I/O and recursion, and
> finishing with a console **diploma project**. Every day is a self-contained set of
> exercises focused on a single core topic.
...
The journey: console & loops → conditionals → arrays → classes & encapsulation →
inheritance, abstraction & interfaces → collections → domain modeling → file I/O →
2D arrays → recursion → maps & a final spatial-query application.
...
and coordinate-geometry logic — the foundation of real location-based services.
